...
class GaussianModel:
    ...
    def load_ply(self, path):
       ...
       # labels = np.asarray(plydata.elements[0]["label"])  # 新增读取label列的代码
        ...
        # self._labels = nn.Parameter(torch.tensor(labels, dtype=torch.float, device="cuda").requires_grad_(True))  # 新增标签参数
