...
#
from builtins import enumerate, len, print, range # type: ignore
import imageio
import numpy as np
import torch
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, cam_type):
    render_path = os.path.join(model_path, name, "ours_{}".format(iteration), "renders")#存储路径
    gts_path = os.path.join(model_path, name, "ours_{}".format(iteration), "gt")

    makedirs(render_path, exist_ok=True)
    makedirs(gts_path, exist_ok=True)#存储路径
    render_images = []
    gt_list = []
    render_list = []
    # breakpoint()
    ...
    new_views = views.clone()  #
    print(len(new_views))
    new_views.duplicate_and_append()#前后重复视角
    print("new_views***************************",len(new_views))
    split_points = [i / (len(new_views) - 1) for i in range(len(new_views))]
    ...
    #for idx, view in enumerate(new_views):
        ...
        time_render=split_points[idx]
        rendering = render(view, time_render,gaussians, pipeline, background,cam_type=cam_type,)["render"]
    ...
    print("FPS:",(len(new_views)-1)/(time2-time1))
